...
class Element:
    def __init__(self, gst_element):
        ...
        print self._gst_element.get_name()
        self.name = self._gst_element.get_factory().get_name()+" - "+self._gst_element.get_name()
        ...
        for property in _properties_list:
                if not property.flags & gobject.PARAM_READABLE:
                    logger.info("Property %s is not readable, skipping" %property.name)
